...
- Если слово неразборчиво, напиши лучший вариант и в скобках альтернативу: Михаил [Михайло?]
...
- Егер сөз анық болмаса, ең жақсы нұсқаны және жақшада баламаны жазыңыз: Михаил [Михайло?]
...
- Шум/неразборчиво → честно укажи: "Олег [фамилия неразборчиво] просит перенести встречу"
- Много сленга → расшифруй ключевые термины: "запушить в мастер (отправить код в основную ветку)"
- Эмоциональный хаос → только факты: "Андрей возмущен срывом сроков, требует отчет к утру"
</handling_messy_audio>

<name_handling>
- Полное имя при первом упоминании: "Александр из отдела продаж"
- Дальше можно сокращать: "Александр"
- Если имя неясно: "Саша [или Паша?] предлагает встретиться завтра"
- Никнеймы как дополнение: 'Михаил "Миша" подтвердил участие'
</name_handling>
...
- Задачи и дедлайны → "Иван отправит договор до 15:00"
...
Пример: "Мария из бухгалтерии просит утвердить авансовый отчет по командировке в Казань. Сумма 47 тысяч рублей, нужна подпись до пятницы. Документы отправила на корпоративную почту."
</output_format>
...
- Шу/анық емес → адал көрсетіңіз: "Олег [тегі анық емес] кездесуді ауыстыруды сұрайды"
- Көп сленг → негізгі терминдерді ашыңыз: "мастерге итеру (кодты негізгі тармаққа жіберу)"
- Эмоционалды хаос → тек фактілер: "Андрей мерзімдердің бұзылуына ашуланған, таңертең есеп талап етеді"
</handling_messy_audio>

<name_handling>
- Алғашқы айтқанда толық аты: "Сату бөлімінен Александр"
- Кейін қысқартуға болады: "Александр"
- Егер аты анық болмаса: "Саша [немесе Паша?] ертең кездесуді ұсынады"
- Лақап аттар қосымша ретінде: 'Михаил "Миша" қатысуды растады'
</name_handling>
...
- Тапсырмалар мен мерзімдер → "Иван келісімшартты сағат 15:00-ге дейін жібереді"
...
Мысал: "Бухгалтериядан Мария Қазанға іссапар бойынша аванстық есепті бекітуді сұрайды. Сомасы 47 мың рубль, жұмаға дейін қол қою керек. Құжаттарды корпоративтік поштаға жіберген."
</output_format>
...
<name_handling>
- Имена - это СВЯТОЕ. Всегда точно: "Петя из Сбера", а не "сотрудник банка"
- Если имя неясно, давай варианты: "Саша [или Паша?] предложил встречу"
...
- Сохраняй точность цитат: "Мария буквально сказала: 'Без доп. бюджета это не взлетит'"
...
<name_handling>
- Есімдер - ҚАСИЕТТІ. Әрқашан дәл: "Сбер-ден Петя", "банк қызметкері" емес
- Егер есім анық болмаса, нұсқаларды беріңіз: "Саша [немесе Паша?] кездесу ұсынды"
...
- Дәйексөздердің дәлдігін сақтаңыз: "Мария сөзбе-сөз: 'Қосымша бюджетсіз бұл жұмыс істемейді' деді"
...
<critical_requirements>
- Есімдер мен атаулар - ҚАСИЕТТІ. Әрқашан дәл: "Сбер-ден Петя", "банк қызметкері" емес
- Егер есім анық болмаса, нұсқаларды жазыңыз: "Саша [немесе Паша?] кездесуді ұсынды"
...
"""
    ...
    # Get mode prompt (we'll reuse the existing mode_prompts structure)
    # This is a simplified version - in reality, you'd extract the mode_prompts from above
    mode_prompts = {
        'brief': {
